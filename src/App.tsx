    ...
    <main>
      ...
      <section className="catalog container" id="catalog">
        ...
        <div className="product-grid">
          {products.map((product) => <article className="product-card" key={product.name}>
            <div className={`product-image ${product.tone} ${product.image ? 'is-zoomable' : ''}`} onClick={() => product.image && setExpandedProduct(product)} onKeyDown={(event) => { if (product.image && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); setExpandedProduct(product); } }} role={product.image ? 'button' : undefined} tabIndex={product.image ? 0 : undefined} aria-label={product.image ? `Рассмотреть майку ${product.name}` : undefined}>{product.image ? <img className="product-photo" src={product.image} alt={`Майка ${product.name}`} /> : <div className="shirt"><div className="collar" /><span>МЕЖА</span></div>}<span className="product-number">{product.mark} / 04</span><span className="product-stamp">МЕЖА<br />MADE IN BY</span>{product.image && <span className="zoom-hint">нажми, чтобы рассмотреть</span>}</div>
      ...
      <section className="order-section container" id="order">
        ...
        <div className="order-form-wrap">
          {submitted ? <div className="success-message"><div className="success-icon"><Check size={28} /></div><h3>Заявка принята.</h3><p>Спасибо. Мы уже получили твой выбор и напишем в Telegram для подтверждения.</p><button className="button button-dark" onClick={() => setSubmitted(false)}>Оформить ещё одну <ArrowRight size={17} /></button></div> : <form className="order-form" onSubmit={submitOrder}>
      ...
      {expandedProduct && expandedProduct.image && <div className="image-modal" role="dialog" aria-modal="true" aria-label={`Просмотр майки ${expandedProduct.name}`} onClick={() => setExpandedProduct(null)}><button className="image-modal-close" onClick={() => setExpandedProduct(null)} aria-label="Закрыть просмотр"><X size={24} /></button><div className="image-modal-content" onClick={(event) => event.stopPropagation()}><img src={expandedProduct.image} alt={`Майка ${expandedProduct.name} — увеличенный просмотр`} /><div><span>{expandedProduct.mark} / 04</span><strong>{expandedProduct.name}</strong></div></div></div>}
